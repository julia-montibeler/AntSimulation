...
void Formiga::draw(Renderer *r){
    ...
    if (this->formiga_tex == NULL) {
        SDL_Surface* formiga_sur = IMG_Load("formiga.png");
        formiga_tex = SDL_CreateTextureFromSurface(r->get_sdlRenderer(), formiga_sur);
    }

    SDL_RenderCopy(r->get_sdlRenderer(), this->formiga_tex, NULL, &rect);
